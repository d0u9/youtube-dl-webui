...
new Vue({
    ...
    methods: {
        getTaskList: function () {
            let _self = this, url = _self.headPath + 'task/list';
            url = url + '?state=' + _self.status;
            Vue.http.get(url).then(function (res) {
                let resData = res.body;
                _self.videoList = resData.detail;
                _self.stateCounter = resData.state_counter;
                _self.stateCounter.all = _self.stateCounter.downloading +
                    _self.stateCounter.finished +
                    _self.stateCounter.paused +
                    _self.stateCounter.invalid;
            }, function (err) {
        ...
        showAddTaskModal: function () {
            let _self = this;
            _self.modalData.add.url = '';
            _self.showModal = true;
            _self.modalType = 'addTask';
            console.log(this.modalData);
            _self.$nextTick(function () {
                _self.$refs.url.focus();
            });
        },
        ...
        addTask: function () {
            let _self = this;
            let url = _self.headPath + 'task';
            for (let key in _self.modalData.add.ydl_opts) {
        ...
        removeTask: function () {
            let _self = this;
            let url = _self.headPath + 'task/tid/' + (_self.videoList[_self.currentSelected] && _self.videoList[_self.currentSelected].tid);
        ...
        pauseTask: function () {
            let _self = this;
            let url = _self.headPath + 'task/tid/' + (_self.videoList[_self.currentSelected] && _self.videoList[_self.currentSelected].tid) + '?act=pause';
        ...
        resumeTask: function () {
            let _self = this;
            let url = _self.headPath + 'task/tid/' + (_self.videoList[_self.currentSelected] && _self.videoList[_self.currentSelected].tid) + '?act=resume';
            Vue.http.put(url).then(function (res) {
        ...
        preference: function () {
            ...
            Vue.http.get(url).then(function (res) {
                let responseJSON = res.body;
                if (responseJSON.status === 'error') {
                    return false;
                } else {
                    let config = responseJSON['config'];
        ...
        bitsToHuman: function (value) {
            let tmp = value, count = 0;
            let metricList = [' B', ' KB', ' M', ' G', ' T', ' P', ' E', ' Z'];
